...
```
toNfrom-helper : ∀ {A B : Set} → A ⊎ B → B ⊎ A
toNfrom-helper (inj₁ x) = inj₂ x
toNfrom-helper (inj₂ x) = inj₁ x

⊎-comm : ∀ {A B : Set} → A ⊎ B ≃ B ⊎ A
⊎-comm = record
         { to = λ{ x → toNfrom-helper x}
         ; from = λ{ x → toNfrom-helper x}
...
currying' {A} {B} {C} =
  record
    ...
    ; from    =  λ{ g → λ{ x → λ{ y → g ⟨ x , y ⟩ }}}
    ; from∘to =  λ{ f → refl }
    ; to∘from =  λ{ g → extensionality {A × B} {C} {_≃_.to ∘ _≃_.from} {λ x_→x} λ{ ⟨ x , y ⟩ → refl }}
    }
```
```
...
```
--this isn't true (have only A, but don't have C)
⊎-weak-×'from : ∀ {A B C : Set} → A ⊎ (B × C) → (A ⊎ B) × C
⊎-weak-×'from (inj₁ a) = {!!}
⊎-weak-×'from (inj₂ b ) = {!!}
```
...
```
--inverse doesn't hold. Take A and D hold but B and C don't
⊎×-implies-×⊎' : ∀ {A B C D : Set} → (A × B) ⊎ (C × D) → (A ⊎ C) × (B ⊎ D)
